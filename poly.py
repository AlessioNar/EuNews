...
## Defining generic class source
class Source():
    def __init__(self, slug, url, status):
        self.slug = slug
        self.url = url
        self.status = status
    ...
    def get_soup(self, container):
        container = container.get_attribute('innerHTML')
        soup = bs4.BeautifulSoup(container, 'lxml')
        return soup

    def standardize_date(self, pub_date):
        pub_date = dateparser.parse(pub_date)
        pub_date = np.datetime64(pub_date).astype(datetime)
        return pub_date

    def create_df(self, list):
        # Create empty dataframe
        df = pd.DataFrame(columns =['title', 'pub_date', 'snippet', 'url'])
        # Iterate over article objects and concatenate dataframes
        for article in list:
            temp_df = pd.DataFrame([[article.title,article.pub_date,article.snippet,article.url]], columns =['title', 'pub_date', 'snippet', 'url'])
            df = pd.concat([df, temp_df]).reset_index(drop=True)
        return df


## Defining child class Apre
class Apre(Source):

    def get_container(self, driver):
        container = driver.find_element_by_xpath('//main[@class="site-main"]')
        return container

    def get_soup(self, container):
        return super().get_soup(container)

    # Parse html and put it in a dataframe
    def get_articles(self, soup):
        ...
        list = [None] * len(list_item)

        for index, item in enumerate(list_item):
            article = Article()
            article.set_title(item.text.strip())
            article.set_url(item.a['href'])
            list[index] = article

        list_item = soup.find_all('span', {'class':'published'})
        for index, item in enumerate(list_item):
            tmp = item.text.strip()
            pub_date = super().standardize_date(tmp)
            list[index].set_pub_date(pub_date)

        list_item = soup.find_all('div', {'id':'bottom-blog'})
        for index, item in enumerate(list_item):
            list[index].set_snippet(item.text.strip())

        df = super().create_df(list)
        return df

class Areflh(Source):

    def get_container(self, driver):
        container = driver.find_elements_by_xpath('//div[@class="uk-container"]')[1]
        return container
    ...
    def get_articles(self, soup):
        ...
        for index, item in enumerate(list_item):
            list[index].set_url('https://www.areflh.org' + item['href'])


        list_item = soup.find_all("div", {"class": "el-meta uk-text-meta uk-margin-top"})
        for index, item in enumerate(list_item):
            tmp = re.sub('Publié le', '', item.text).strip()
            pub_date = super().standardize_date(tmp)
            list[index].set_pub_date(pub_date)
        ...
        for index, item in enumerate(list_item):
            if item.p:
                list[index].set_snippet(item.p.text)
            else:
                list[index].set_snippet(item.div.text)
